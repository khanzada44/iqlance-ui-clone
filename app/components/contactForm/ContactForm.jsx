...
export default function ContactForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response && (response.status === 200 || response.success || response.status === 201)) {
        ...
        setStatus({ type: "error", text: response?.message || "Failed to submit. Please try again." });
      }
    } catch (err) {
      setStatus({ 
        type: "error", 
        text: err?.response?.data?.message || "Something went wrong. Check connection." 
      });
    } finally {
      setLoading(false);
    }
          ...
          <form className="space-y-5" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              <div className="relative">
                <select
                  ...
                  onChange={handleChange}
                  className="w-full bg-white border border-gray-300 px-4 py-3 text-sm text-gray-600 focus:outline-none focus:border-[#184A8B] appearance-none cursor-pointer pr-10"
                >
            ...
            <div className="pt-2">
              <button
                type="submit"
                disabled={loading}
                className="bg-[#184A8B] hover:bg-[#123b72] text-white px-7 py-3.5 text-sm font-bold transition duration-200 cursor-pointer shadow-sm disabled:opacity-50"
              >
